...
int player_turn(struct player *ptr_to_player, struct bot *ptr_to_bot, struct player_cards *ptr_to_player_cards, struct bot_cards *ptr_to_bot_cards, int player_element, struct player_spells *ptr_to_player_spells, int Level)
{
    ...
    if (type == 2)
    {
        ...
        for (i = 0; i < 1 + Level; i++)
        {
            printf(" %d - %[^\n]s", i, ((ptr_to_player_spells + i)->name));
        }
...
void bot_move_decider(struct player *ptr_to_player, struct bot *ptr_to_bot, struct player_cards *ptr_to_player_cards, struct bot_cards *ptr_to_bot_cards, int bot_element, struct bot_spells *ptr_to_bot_spells, int Level, int *ptr_to_arr_of_moves)
{
    ...
    for (i = 0; i < 4 + Level; i++)
    {
        if (ptr_to_player->mana >= (ptr_to_player_cards + i)->manacost)
        {
            if ((ptr_to_player_cards + i)->isblocked == 0)
            {
...
int bot_turn(struct player *ptr_to_player, struct bot *ptr_to_bot, struct player_cards *ptr_to_player_cards, struct bot_cards *ptr_to_bot_cards, int bot_element, struct bot_spells *ptr_to_bot_spells, int Level)
{
    int i;
...
void reallocating_player_cards(struct spells *ptr_to_all_spells, struct cards *ptr_to_all_cards, struct player_cards *ptr_to_player_cards,struct player_spells *ptr_to_player_spells , int Level)
{
    ...
    newSize=4+Level;
    i++;
    ptr_to_player_cards =realloc(ptr_to_player_cards, newSize *sizeof(struct player_cards));
...
if (cardChoice >= 1 && cardChoice <= 12) 
{
    ...
    new_Size = 1;
    new_Size += Level;
    ptr_to_player_spells = realloc(ptr_to_player_spells, new_Size * sizeof(struct player_spells));
...
void reallocating_bot_cards(struct spells *ptr_to_all_spells, struct cards *ptr_to_all_cards, struct player_cards *ptr_to_bot_cards,struct player_spells *ptr_to_bot_spells , int Level)
{
    int newSize=4+Level;
    ptr_to_bot_cards =realloc(ptr_to_bot_cards, newSize *sizeof(struct bot_cards));
    int cardChoice;
    cardChoice = rand() % 12;
            ...
            while (i == 0) // set the winning conditions here
            {
                ...
                if (i == 1)
                {
                    ...
                    while (i == 0) // set the winning conditions here
                    {
                        ...
                        i = bot_turn(ptr_to_player, ptr_to_bot, ptr_to_player_cards, ptr_to_bot_cards, bot_element, ptr_to_bot_spells, Level);
                        if (i == 1)
                        {
                            break;
                        }
